...
class StratigraphicColumn(object):
    def __init__(self):
        ...
        stratigraphicUnitColumns = numpy.dtype([
            ('layerId',int),('name',str),('minAge',float),('maxAge',float),
            ('group',str),('thickness',float),('colour',str)
        ])
        self.stratigraphicUnits = pandas.DataFrame(numpy.empty(0,dtype=stratigraphicUnitColumns))
        ...
        self.lithologyUnits = pandas.DataFrame(numpy.empty(0,dtype=lithologyUnitColumns))
