...
def plot_grid(grid, axis, title):
    axis.imshow(
        grid,
        cmap="gist_rainbow",
        vmin=np.percentile(grid, 1.5),
        vmax=np.percentile(grid, 98.5),
    )
    axis.title.set_text(title)
    # plt.show()
...
def plot_density(density, axis):
    if len(density) > 0:
        ...
        plt.text(1.2, 0.2, "No density values\n in area")
    plt.tight_layout()
...
def get_dtm_profile(minlong, minlat, maxlong, maxlat, mag_long, mag_lat, l):
    ...
    dtm_file = "./dtm.tif"
    bbox = (minlong - 0.1, minlat - 0.1, maxlong + 0.1, maxlat + 0.1)
...
def GA_strat_petrophysics(
    petrophysics, geology, mag_sus_filename_local, density_filename_local
):
            ...
            if petrophysics.loc[g]["ms_samples"] == 0:
                if len(g_ms) > 0:
                    ...
                    if len(g_ms) == 1:
                        ...
                        petrophysics.at[g, "ms_std"] = np.log(
                            g_ms["value"] + 0.00001
                        ).std()
    ...
    for s in no_ms_strat:
        ...
        if s in strat_dic_gswa2.index:
            for ls in list(set(strat_dic_gswa2.loc[s]["exploded"])):
                ...
                GA_ms_l = GA_ms[
                    GA_ms["lithology"].str.contains(ls) & GA_ms["value"] > 0
                ]
            ...
            if len(GA_ms_l_all) > 0:
                ...
                if len(GA_ms_l_all) == 1:
                    ...
                    petrophysics.at[s, "ms_std"] = np.log(
                        np.abs(GA_ms_l_all["value"])
                    ).std()
