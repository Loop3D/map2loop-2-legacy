...
def plot_geochron(geochron_filename, bbox):
    ...
    for ind, date in geochron_sort.iterrows():
        ...
        line = plt.Line2D(
            (i * xscale, i * xscale),
            (
                date["ANALYSIS_A"] - date["ANALYSIS_U"],
                date["ANALYSIS_A"] + date["ANALYSIS_U"],
            ),
            lw=1.0,
            color="#000000",
...
def plot_geology_codes(geology_filename, bbox):
    ...
    geol["AVE_AGE_MA"] = geol["MIN_AGE_MA"].astype("float") + (
        (geol["MAX_AGE_MA"].astype("float") - geol["MIN_AGE_MA"].astype("float")) / 2.0
    )
...
def plot_geology_parents(geology_filename, bbox):
    ...
    for parent in parents:
        ...
        for code in geol_unique["CODE"]:
            if geol_unique_code.loc[code]["PARENTNAME"] == parent:
                if (
                    not str(geol_unique_code.loc[code]["MIN_AGE_MA"]) == "None"
                    and not str(geol_unique_code.loc[code]["MAX_AGE_MA"]) == "None"
                ):
    ...
    for parent in minmax_parent_sort:
        ...
        rectangle = plt.Rectangle(
            (i, minmax_parent[key][0]),
            xscale,
            minmax_parent[key][1] - minmax_parent[key][0],
            fc="C" + str(c),
            edgecolor="k",
            label=key,
        )
...
def plot_linear_segments(linear_filename, bbox, feature):
    ...
    if nSegs > 0:
        ...
        plt.bar(
            np.deg2rad(np.arange(0, 360, nBinWidth)),
            n,
            width=np.deg2rad(nBinWidth),
            bottom=0.0,
            color=".8",
            edgecolor="k",
        )
        ...
        plt.xticks(
            np.radians(range(0, 360, 45)),
            ["0", "45", "90", "135", "180", "215", "270", "315"],
        )
        if nMax > 6:
            plt.rgrids(
                range(0, int(round(nMax * 1.1)), int(round((nMax * 1.1) / 5))),
                angle=330,
            )
...
def plot_eggs(eggs_filename, bbox):
    ...
    if len(eggs) > 0:
        ...
        plt.text(0.2, 0.2, "No eggs values\n in area")
    plt.title("EGGS")
    plt.show()
...
def plot_density(density_filename, bbox):
    ...
    if len(density) > 0:
        ...
        plt.text(0.2, 0.2, "No density values\n in area")
    plt.tight_layout()
    plt.title("Density")
    plt.show()
    return density
...
def plot_mag_sus(mag_sus_filename, bbox):
    ...
    if len(mag_sus) > 0:
        ...
        plt.text(0.2, 0.2, "No mag sus values\n in area")
    plt.tight_layout()
...
def plot_plutons(geology_filename, bbox):
    ...
    for ind, poly in plutons_sort_unique.iterrows():
        if (
            ...
            ave_age = (
                float(poly["MIN_AGE_MA"])
                + (float(poly["MAX_AGE_MA"]) - float(poly["MIN_AGE_MA"])) / 2.0
            )
