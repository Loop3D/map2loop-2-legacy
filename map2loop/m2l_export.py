...
def loop2geomodeller(model_name, test_data_path, tmp_path, output_path, dtm_file, bbox, model_top, model_base, save_faults, compute_etc, workflow):
    ...
    all_sorts = np.genfromtxt(
        tmp_path+'all_sorts_clean.csv', delimiter=',', dtype='U100')
    ...
    if(save_faults):
        ...
        for i in range(0, len(edges)):
            ...
            if(not found):
                ostr = '        FaultStopsOnFaults{ fault: "' + \
                    edges[i][1]+'"; stopson: "'+edges[i][0]+'"}\n'
                f.write(ostr)
    ...
    if(save_faults):
        all_fault_group = np.genfromtxt(
            output_path+'group-fault-relationships.csv', delimiter=',', dtype='U100')
        ...
        for i in range(1, nfaults):
            ...
            for j in range(1, ngroups):
                if(all_fault_group[i, j] == str(1)):
                    if(first):
                        ostr = '    FaultSeriesLinks{ fault: "' + \
                            all_fault_group[i, 0]+'"; series: ['
                        f.write(ostr)
...
def loop2geomodeller2(model_name, test_data_path, tmp_path, output_path, dtm_file, bbox, save_faults, compute_etc, workflow):
    ...
    for inx, afm in all_sorts.iterrows():
        ...
        if(not afm['code'] in empty_fm):
            ...
            for indx2, acontact in contacts.iterrows():
                if(acontact['formation'] in afm['code']):
                    ostr2 = '              point {x:'+str(acontact['X'])+'; y:'+str(
                        acontact['Y'])+'; z:'+str(acontact['Z'])+'}\n'
                    ostr.append(ostr2)
            ostr.append('    }\n')
            ostr.append('}\n')
        ...
        for i in range(0, len(edges)):
            ...
            for j in range(0, len(cycles)):
                if(edges[i][0] == cycles[j][0] and edges[i][1] == cycles[j][1]):
                    found = True  # fault pair is first two elements in a cycle list so don't save to taskfile
            if(not found):
                ostr2 = '        FaultStopsOnFaults{ fault: "' + \
                    edges[i][1]+'"; stopson: "'+edges[i][0]+'"}\n'
                ostr.append(ostr2)
...
def loop2LoopStructural(m2l_directory):
    ...
    for f in m2l_data['max_displacement'].keys():
        ...
        overprints = []
        try:
            overprint_id = m2l_data['fault_fault'][m2l_data['fault_fault']
                                                   [fault_id] == 1]['fault_id'].to_numpy()
    ...
    for i in m2l_data['groups']['group number'].unique():
        g = m2l_data['groups'].loc[m2l_data['groups']
                                   ['group number'] == i, 'group'].unique()[0]
...
def loop2gempy(test_data_name: str, tmp_path: str, vtk_path: str, orientations_file: str,
    ...
    for group in contents:
        ...
        surfaces_g = np.atleast_2d(np.genfromtxt(
            tmp_path + group + '.csv', delimiter=',', dtype='U100'))
    ...
    if('Default series' in map_series_to_surfaces):
        ...
        del_surfaces = geo_model.surfaces.df.groupby(
            'series').get_group('Default series')['surface']
    ...
    if compute is True:
        gp.set_interpolator(
            geo_model, theano_optimizer='fast_run', dtype='float64')
...
def loop2gempy_(test_data_name, tmp_path, vtk_path, orientations_file, contacts_file, groups_file, dtm_reproj_file,
    ...
    for i in range(0, ngroups):
        contents = np.genfromtxt(
            tmp_path+groups[i]+'.csv', delimiter=',', dtype='U100')
    ...
    single_vals = geo_model.surface_points.df[geo_model.surface_points.df['id'].isin(
        id_only_one)]
    for idx, vals in single_vals.iterrows():
        geo_model.add_surface_points(
            vals['X'], vals['Y'], vals['Z'], vals['surface'])
    ...
    # Save model as vtk
    if(vtk):
        gp.plot.export_to_vtk(geo_model, path=vtk_path, name=test_data_name +
                              '.vtk', voxels=False, block=None, surfaces=True)
...
def display_LS_map(model, dtm, geol_clip, faults_clip, dst_crs, use_cmap, cmap, use_topo, use_faults):
    ...
    if(not use_cmap):
        cmap = rand_cmap(100, type='soft', first_color_black=False,
    ...
    points = np.array([xx.flatten(order='F'), yy.flatten(
        order='F'), zz.flatten(order='F')]).T
    ...
    new_dataset = memfile.open(driver='GTiff',
                               ...
                               crs=dst_crs,
                               transform=transform)
    new_dataset.write(v.astype('float64').reshape(
        dtm_val.shape[0], dtm_val.shape[1]), 1)

    fig, ax = matplotlib.pyplot.subplots(figsize=(15, 15))
    rasterio.plot.show(new_dataset.read(
        1), transform=new_dataset.transform, cmap=cmap, ax=ax)
    geol_clip.plot(ax=ax, facecolor='none', edgecolor='black', linewidth=0.4)
    if(use_faults):
        faults_clip.plot(ax=ax, facecolor='none',
