...
#
# save_basal_no_faults(path_out,path_fault,ls_dict,dist_buffer,c_l,proj_crs)
# Args:
...
def save_basal_no_faults(path_out, path_fault, ls_dict, dist_buffer, c_l, proj_crs):
    ...
    if(os.path.exists(path_fault)):
        ...
        contacts = GeoDataFrame(df, crs=proj_crs, geometry='geometry')
        ...
        contacts_nf = GeoDataFrame(df_nf, crs=proj_crs, geometry='geometry')
...
#
# save_contacts_with_faults_removed(path_fault,path_out,dist_buffer,ls_dict,ls_dict_decimate,c_l,proj_crs,dtm)
# Args:
# path_fault path to clipped fault layer
# path_out directory of output csv file
# dist_buffer distance in projection units of buffer around faults to clip
...
def save_contacts_with_faults_removed(path_fault, path_out, dist_buffer, ls_dict, ls_dict_decimate, c_l, proj_crs, dtm, dtb, dtb_null, cover_map):
    ...
    contacts_nf_deci = GeoDataFrame(df_nf, crs=proj_crs, geometry='geometry')
...
# save_fold_axial_traces_orientations(path_folds,output_path,tmp_path,dtm,c_l,proj_crs,fold_decimate,fat_step,close_dip)
# Args:
...
def save_fold_axial_traces_orientations(path_folds, output_path, tmp_path, dtm, dtb, dtb_null, cover_map, c_l, proj_crs, fold_decimate, fat_step, close_dip, scheme, bbox, spacing, dip_grid, dip_dir_grid):
                            ...
                            if(m2l_utils.mod_safe(i, fold_decimate) == 0 or i == int((len(fold_ls.coords)-1)/2) or i == len(fold_ls.coords)-1):
                                ...
                                if(not first):
                                    ...
                                    if(dotprod > 0.85):
                                        ...
                                        gdf = GeoDataFrame(
                                            dummy, crs=proj_crs, geometry=geometry)
...
# fault_strat_offset(path_out,c_l,proj_crs,fm_thick_file, all_sorts_file,fault_file,geol_file,fault_dim_file)# Args:
# orientations_path path to orientations file
# path_out path to output directory
# c_l dictionary of codes and labels specific to input geo information layers
# proj_crs Coordinate Reference System of vector files
...
###################################################
def fault_strat_offset(path_out, c_l, proj_crs, fm_thick_file, all_sorts_file, fault_file, geol_file, fault_dim_file):
...
def process_cover(output_path, dtm, dtb, dtb_null, cover, cover_map, cover_dip, bbox, proj_crs, spacing, contact_decimate, use_vector, use_grid):
