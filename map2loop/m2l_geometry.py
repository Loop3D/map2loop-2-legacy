...
def save_faults(path_faults, output_path, dtm, dtb, dtb_null, cover_map, c_l, fault_decimate, fault_min_len, fault_dip):
    ...
    fd.write("Fault,HorizontalRadius,VerticalRadius,InfluenceDistance,incLength,colour\n")
                    ...
                    if(strike > fault_min_len):
                        ...
                        for afs in flt_ls.coords:
                            if(first):
                                ...
                                incLength = incLength+m2l_utils.ptsdist(afs[0],afs[1],lastx,lasty)
                                lastx = afs[0]
                                lasty = afs[1]
                        ...
                        locations  =  [(flt_ls.coords[len(flt_ls.coords)-1][0], flt_ls.coords[len(flt_ls.coords)-1][1])]
                        height  =  m2l_utils.value_from_dtm_dtb(
                                    dtm, dtb, dtb_null, cover_map, locations)
                        ostr  =  "{},{},{},{},{},{},{}\n"\
                    ...
                    if(not str(acontact.geometry) == 'None'):
                        ...
                        if(ddline.intersects(acontact.geometry)): 
                            ...
                            if(i>0): #if we found any intersections with base of next higher unit
                                ...
                                if(min_dist<max_thickness_allowed and min_dist>0): #if not too far, add to output
                                    locations = [(cx[k],cy[k])]
                                    zbase = float(m2l_utils.value_from_dtm_dtb(dtm,'','',False,locations))
                                    locations = [(crossx,crossy)]
                                    zcross = float(m2l_utils.value_from_dtm_dtb(dtm,'','',False,locations))
                                    delz = fabs(zcross-zbase)
                                    slope_dip = degrees(atan(delz/min_dist))
                                    slope_length = sqrt((min_dist*min_dist)+(delz*delz))
